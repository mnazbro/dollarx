...
public class CustomPropertiesTest extends XPathTester {
    ...
    public void customElementPropertySingleParam() {
        Function<String, ElementProperty> role =
                createPropertyGenerator(value -> format("@role='%s'", value), value -> format("has role %s", value));
        Path el = div.that(hasProperty(role, "foo")).and(hasClass("x"));
        NodeList nodes = findAllByXpath(
                "<div class=\"a\">x</div><div class=\"b\">x</div><div class=\"x\" role=\"foo\">x</div><div role=\"bar\" class=\"d\">x</div>",
                el);
    ...
    public void customElementPropertyTwoParams() {
        ...
        NodeList nodes = findAllByXpath(
                "<div data-foo=\"3\" class=\"a\">x</div><div class=\"b\">x</div><div class=\"x\" data-foo=\"4\">x</div><div role=\"bar\" class=\"d\">x</div>",
                el);
        assertThat(nodes.getLength(), is(1));
        assertThat(getCssClass(nodes.item(0)), is("x"));
