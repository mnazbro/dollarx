...
public class SimilarityComparator implements BiConsumer<BufferedImage, BufferedImage> {
    ...
    private static boolean noTransitionFound(
            int actualPixOneSide, int actual, int actualPixOtherSide, int refPixOneSide, int ref, int refPixOtherSize) {
        ...
        return !foundTransition
                || ImageUtils.avgIsSignificantlyDifferent(
                        actualPixOneSide, actual, actualPixOtherSide, refPixOneSide, ref, refPixOtherSize);
    }
    ...
    public void accept(BufferedImage refImage, BufferedImage actualImage) {
        ...
        Images.logger.info(
                format("found %d significant differences. This is under the %d threshold", countOfErrors, threshold));
    }
    ...
    public static Optional<BufferedImage> getErrorImage(BufferedImage refImage, BufferedImage actualImage) {
        ...
        BufferedImage errImage =
                new BufferedImage(refImage.getWidth(), refImage.getHeight(), BufferedImage.TYPE_INT_RGB);
        for (int y = 1; y < actualImage.getHeight() - 1; y++) {
            for (int x = 1; x < actualImage.getWidth() - 1; x++) {
                if (pixelMismatch(actualImage, refImage, x, y)) {
                    foundDiff.set(true);
