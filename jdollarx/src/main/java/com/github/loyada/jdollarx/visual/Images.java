...
public class Images {
     ...
     * @throws IOException - file could not be read
     */
    public static void assertImageIsEqualToExpectedNoScaling(InBrowser browser, Path el, InputStream expectedImageInput)
            throws IOException {
     ...
     * @throws IOException - file could not be read
     */
    public static void assertImageIsEqualToExpected(BufferedImage img, InputStream expectedImageInput)
            throws IOException {
     ...
     * @throws AssertionError - images are not the same size
     */
    public static Optional<BufferedImage> getFuzzyErrorsImage(
            InBrowser browser, Path el, InputStream expectedImageInput) throws IOException {
        BufferedImage elementImage = captureImage(browser, el);
        BufferedImage expectedImage = ImageIO.read(expectedImageInput);
     ...
     */
    public static Optional<BufferedImage> getFuzzyErrorsImage(
            InputStream actualImageInput, InputStream expectedImageInput) throws IOException {
        BufferedImage expectedImage = ImageIO.read(expectedImageInput);
     ...
     * @throws AssertionError - images are not the same size
     */
    public static Optional<BufferedImage> getFuzzyErrorImage(InBrowser browser, Path el, InputStream expectedImageInput)
            throws IOException {
        BufferedImage elementImage = captureImage(browser, el);
        BufferedImage expectedImage = ImageIO.read(expectedImageInput);
        return ImageComparator.getFuzzyErrorImage(elementImage, expectedImage);
     ...
     * @throws IOException - file could not be read
     */
    public static void assertImageIsEqualToExpectedWithShiftAndCrop(
            InBrowser browser, Path el, InputStream expectedImageInput, int maxShift) throws IOException {
        BufferedImage elementImage = captureImage(browser, el);
        BufferedImage expectedImage = ImageIO.read(expectedImageInput);
     ...
     */
    public static void assertImageIsSimilarToExpected(
     ...
     * @throws IOException - image file could not be read
     */
    public static void assertWindowIsSimilarToExpected(
            InBrowser browser, InputStream expectedImageInput, int maxBadPixelsRatio) throws IOException {
        BufferedImage elementImage = captureImage(browser);
        BufferedImage expectedImage = ImageIO.read(expectedImageInput);
     ...
     * @throws IOException - image file could not be read
     */
    public static void assertImageIsSimilarToExpectedWithShift(
    ...
    public static void assertImageIsSimilarToExpectedWithFilter(
        ...
        ImageComparator.verifyImagesAreSimilarFilteringInterestingAreas(
                filterImage, expectedImage, elementImage, maxBadPixelsRatio);
    }
    ...
    public static void assertWindowIsSimilarToExpectedWithFilter(
            InBrowser browser, InputStream filterImageInput, InputStream expectedImageInput, int maxBadPixelsRatio)
            throws IOException {
        BufferedImage elementImage = captureImage(browser);
        BufferedImage expectedImage = ImageIO.read(expectedImageInput);
        BufferedImage filterImage = ImageIO.read(filterImageInput);
    ...
    public static BufferedImage captureCanvas(InBrowser browser, Path canvas) {
        ...
        String imageEncondedData = (String) js.executeScript(
                "return arguments[0].toDataURL('image/png').substring(arguments[1]);", canvasEl, startOfDataInDataURL);
    ...
    private static Point getVisiblePageOffset(InBrowser browser) {
        ...
        Map<String, Long> pointResult = (Map<String, Long>) js.executeScript(
                "return {'y': parseInt(window.pageYOffset, 10), 'x': parseInt(window.pageXOffset, 10)};");
    ...
    public static Map<String, Long> getVisibleDimensions(InBrowser browser, Path path) {
        ...
        return castToMap(js.executeScript(
                "return  { 'height':  parseInt(arguments[0].clientHeight, 10), 'width':   parseInt(arguments[0].clientWidth, 10)};",
                el));
    }
    ...
    public static Map<String, Long> getBrowserInnerDimensions(InBrowser browser) {
        ...
        return castToMap(
                js.executeScript(
                        "return  { 'height':  parseInt(Math.min(window.innerHeight, outerHeight), 10), 'width':   parseInt(Math.min(window.innerWidth, window.outerWidth), 10)};"));
    }
    ...
    public static BufferedImage captureImage(InBrowser browser, Path el) {
        ...
        if (fullImg.getWidth() + fullImgOffset.getX() < elementLocation.getX() + elementDimensions.getWidth()
            ...
            throw new IllegalArgumentException(format(
                    "The element '%s' is partially outside the visible area in the browser."
                            + "You might need to resize the window to a larger size, or scroll to the location of the element",
                    el));
        }
