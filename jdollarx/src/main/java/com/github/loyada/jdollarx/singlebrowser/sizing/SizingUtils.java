...
class SizingUtils {
    ...
    */
    ...
    static void setDimensions(Path path, String width, String height) {
        ...
        String script =
                format("arguments[0].style.width=%s;  arguments[0].style.height=%s;", correct(width), correct(height));
    ...
    static Map<String, Long> getVisibleDimensions(Path path) {
        ...
        return castToMap(js.executeScript(
                "return  { 'height':  arguments[0].clientHeight, 'width':   arguments[0].clientWidth};", el));
    }
    ...
    static Map<String, Long> getScrollableDimensions(Path path) {
        ...
        WebElement el = find(path);
        return castToMap(js.executeScript(
                "return  { 'height':  arguments[0].scrollHeight, 'width':   arguments[0].scrollWidth};", el));
    }
    ...
    static Map<String, String> getStylingDimensions(Path path) {
        ...
        WebElement el = find(path);
        return castToMap(js.executeScript(
                "return  { 'height':  arguments[0].style.height, 'width':   arguments[0].style.width};", el));
    }
