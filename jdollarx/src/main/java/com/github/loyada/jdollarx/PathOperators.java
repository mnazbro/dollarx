...
public final class PathOperators {
    ...
    public static Path not(Path path) {
        ...
        return BasicPath.builder()
                .withXpath(String.format(
                        "*[not(self::%s)]",
                        PathUtils.transformXpathToCorrectAxis(path).get()))
                .withUnderlyingOptional(path.getUnderlyingSource())
                .withAlternateXpath(String.format(
                        "*[not(self::%s)]", path.getAlternateXPath().get()))
                .withXpathExplanationOptional(Optional.of(String.format("anything except (%s)", path)))
                .build();
    }
}
